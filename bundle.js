...
var Drawflow = exports["default"] = /*#__PURE__*/function () {
  ...
  return _createClass(Drawflow, [{
    ...
    value: function updateNodeDataFromId(id, data) {
      ...
      if (this.module === moduleName) {
        var _insertObjectkeys = function insertObjectkeys(object, name, completname) {
          if (object === null) {
            var object = data[name];
          } else {
            var object = object[name];
          ...
          if (object !== null) {
            Object.entries(object).forEach(function (key, value) {
              if (_typeof(key[1]) === "object") {
                _insertObjectkeys(object, key[0], completname + '-' + key[0]);
              } else {
                var elems = content.querySelectorAll('[df-' + completname + '-' + key[0] + ']');
                for (var i = 0; i < elems.length; i++) {
                  elems[i].value = key[1];
        ...
        Object.entries(data).forEach(function (key, value) {
          if (_typeof(key[1]) === "object") {
            _insertObjectkeys(null, key[0], key[0]);
          } else {
            var elems = content.querySelectorAll('[df-' + key[0] + ']');
            for (var i = 0; i < elems.length; i++) {
              elems[i].value = key[1];
...
var _Drawflow = _interopRequireDefault(require("./Drawflow"));
function _interopRequireDefault(e) { return e && e.__esModule ? e : { "default": e }; }
// index.js
// adjust the path if needed
window.Drawflow = _Drawflow["default"]; // Expose Drawflow globally
